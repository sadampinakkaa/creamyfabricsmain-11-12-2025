...
// Consolidated variant change handler
document.addEventListener('variant:changed', function(event) {
	let currentVariant = event.detail.variant;
	
	// Update all variant-related functions
...
var updateProductMedia = (variantId) => {
	...
	document.querySelectorAll(`[js-product-media-id]`).forEach(element => {
		if (String(element.getAttribute('js-product-media-id')).includes(String(variantId))) {
			...
			// Reinitialize flickity for the active gallery and go to first slide
			const flickityCarousel = element.querySelector('flickity-carousel');
			if (flickityCarousel && flickityCarousel.flickityInstance) {
				flickityCarousel.flickityInstance.then(flkty => {
					if (flkty) {
						flkty.resize();
						// Always show first image when switching variants (size changes within same color)
